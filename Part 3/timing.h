// This file was used from the internet to help time the different processes
...
#  include <sys/time.h>
  ...
  public:
    ...
    static SysClock currentTicks() {
      ...
      return Timer::SysClock(static_cast<float>(spec.tv_sec) * 1e9 + static_cast<float>(spec.tv_nsec));
#endif
    }
    ...
    static double secondsPerTick() {
      ...
      char input[1024];
      if (!fp) {
         fprintf(stderr, "Timer::resetScale failed: couldn't find /proc/cpuinfo.");
         exit(-1);
      }
